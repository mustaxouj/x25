...
public class ServiceAdmin extends Auth implements IServiceAdmin, InteractiveConsole {
    ...
    public Client modifierClient(String filtre) {
        ...
        if (chercherClientParId(id) != null) {
            ServiceClient sC = new ServiceClient(chercherClientParId(id));
    ...
    public List<Client> trierClientParAdresse() {
        ...
        for (Client client : this.getBanque().getClientsDeBanque())
            clients.add(client);
        Collections.sort(clients, (client1, client2) -> client1.getAdresse().compareTo(client2.getAdresse()));
        return clients;
    }
    @Override
